...
import java.util.*;
public class MeningitisCalculation extends AbstractPatientCalculation {
    protected static final Log log = LogFactory.getLog(MeningitisCalculation.class);
    ...
    Integer SUDDEN_FEVER_ONSET = 162707;
    ...
    public CalculationResultMap evaluate(Collection<Integer> cohort, Map<String, Object> parameterValues, PatientCalculationContext context) {
        ...
        for (Integer ptId : alive) {
            ...
            Concept jaundiceResult = cs.getConcept(SUDDEN_FEVER_ONSET);
            Concept screeningQuestion = cs.getConcept(SCREENING_QUESTION);

            boolean patientMeningitisTriageEncResult = lastTriageEnc != null ? EmrUtils.encounterThatPassCodedAnswer(lastTriageEnc, screeningQuestion, jaundiceResult) : false;
            boolean patientMeningitisClinicalEncResult = lastClinicalEncounter != null ? EmrUtils.encounterThatPassCodedAnswer(lastClinicalEncounter, screeningQuestion, jaundiceResult) : false;
            boolean patientMeningitisGreenCardResult = lastGreenCardEnc != null ? EmrUtils.encounterThatPassCodedAnswer(lastGreenCardEnc, screeningQuestion, jaundiceResult) : false;

            if (lastClinicalEncounter != null) {
                for (Obs obs : lastClinicalEncounter.getObs()) {
                    if (patientMeningitisClinicalEncResult) {
            ...
            if (lastGreenCardEnc != null) {
                for (Obs obs : lastGreenCardEnc.getObs()) {
                    if (patientMeningitisGreenCardResult) {
                        dateCreated = obs.getDateCreated();
                        if (dateCreated != null) {
                            String createdDate = dateFormat.format(dateCreated);
                            if (createdDate.equals(todayDate)) {
            ...
            if (lastTriageEnc != null) {
                for (Obs obs : lastTriageEnc.getObs()) {
                    if (patientMeningitisTriageEncResult) {
                        dateCreated = obs.getDateCreated();
                        if (dateCreated != null) {
                            String createdDate = dateFormat.format(dateCreated);
                            if (createdDate.equals(todayDate)) {
